...
$result = mysqli_query($connect, "SELECT COUNT(`id`) as total FROM `personal_messages` WHERE (`from_id` = $user_id AND `to_id` = $contact_id) 
                                        OR (`from_id` = $contact_id AND `to_id` = $user_id)");
$row = mysqli_fetch_assoc($result);
$total_records = $row['total'];
$total_records > 50 ? $start = $total_records - 50 : $start = 0;
$query = mysqli_query($connect,"SELECT `id`, `from_id`, `message` FROM `personal_messages` WHERE (`from_id` = $user_id AND `to_id` = $contact_id) 
...
if (mysqli_num_rows($query)>0) {
    while ($messagearr = mysqli_fetch_assoc($query)) {
        ...
        $query_user = mysqli_query($connect, "SELECT `nickname` FROM `members` WHERE `id` = $mes_from_id");
